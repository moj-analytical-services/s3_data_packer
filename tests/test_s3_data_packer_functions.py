...
def test_get_meta(
    tmp_path, file_format: str, expected_get_meta_return_type: bool, cast_parquet: bool
):
    # set up
    pp = setup_packer(
        tmp_path, cast_parquet=cast_parquet, metadata="tests/data/all_types.json"
    )
...
def test_append_files(
    ...
    expected_output_df = (
        cast_pandas_table_to_schema(expected_output_df, metadata)
        if metadata
        else expected_output_df
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"tests/data/all_types_one_line.csv": ["land/all_types/all_types_a"]}, {}, 1),
        # scenario 1: 1 file in, no existing output
        ({"tests/data/all_types.csv": ["land/all_types/all_types_a"]}, {}, 10),
        ...
        (
            {"tests/data/all_types.csv": ["land/all_types/all_types_a"]},
...
def test_packed_data(
    ...
    if output_filemap:
        output_filemap = {
            k: [f"{f}.{ff}" for f in v] for k, v in output_filemap.items()
        }
    if input_filemap:
