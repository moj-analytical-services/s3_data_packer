...
def test_get_filename(
    ...
    ps = setup_output_store(
        tmp_path, output_file_map, table_suffix=table_suffix
    )
    ps.table_extension = file_format
    # build expected filename
    expected_filename = (
        f"_{table_suffix}_".join(expected_filename.rsplit("_", 1))
        if table_suffix else expected_filename
    )
    expected_filename = f"{expected_filename}.{file_format}"
    expected_filename = os.path.join(
        tmp_path, "db/all_types", expected_filename
    ) if full_path else expected_filename
...
@pytest.mark.parametrize(
    "fn", ["a_file.csv", "another_file_0322_asfas.csv", 0, None]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"tests/data/all_types.csv": ["db/all_types/at_0.snappy.parquet", "db/all_types/at_7.snappy.parquet"]}, 7),
        ({"tests/data/all_types.csv": ["db/all_types/at_0.snappy.parquet", "db/all_types/at_1.snappy.parquet"]}, 1),
        ({"tests/data/all_types.csv": ["db/all_types/at_5.snappy.parquet", "db/all_types/at_7456.snappy.parquet"]}, 7456)
    ]
)
def test_output_store_filenum(
    tmp_path, output_file_map, expected_output_store_filenum
):
    output_store = setup_output_store(
        tmp_path, file_map=output_file_map
    )
...
def test_get_latest_file_by_suffix(tmp_path, file_list, maxim, full_path):
    ...
    expected_file = os.path.join(
        tmp_path, "db/all_types", expected_file
    ) if (full_path and file_list) else expected_file
...
def test_should_append_data(
    tmp_path, output_file_map, file_limit_gigabytes
):
    output_store = setup_output_store(
        tmp_path, file_map=output_file_map,
